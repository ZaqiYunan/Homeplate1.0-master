...
export default function SupabaseLoginPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Login result:', { error: error?.message || 'No error' });
      ...
      if (error) {
        ...
        const errorMessage = error.message || error.msg || error.error_description || 'Unknown error';
        ...
        if (errorMessage.includes('Invalid login credentials') || errorMessage.includes('invalid_grant')) {
          ...
          description = 'Your email confirmation link has expired. Please sign up again or request a new confirmation email.';
          ...
          description = 'Your account may not be confirmed yet. Please check your email for a confirmation link.';
        } else if (errorMessage.includes('Network error') || error.status >= 500) {
